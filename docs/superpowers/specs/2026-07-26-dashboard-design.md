...
| **组件来源** | 引用 Insight（Analysis）内已有 `ViewNode`；因看板独立，ref 需带 `analysisId` |

### 仍待确认

| 决策点 | 选项 | 建议 |
| --- | --- | --- |
| 跨 Insight | 一张看板可否引用 **多个** Analysis 的视图？ | **允许**（细胞培养 / Assay 常是不同 Analysis；独立归属正为此） |
| 组件类型 | P1 表 + 图一起，还是先图？ | 表 + 图 |
| 画布布局 | 12 列拖拽网格 vs 固定模板 | 12 列网格 |
...
1. **跨 Insight**：一张看板是否允许引用多个 Analysis？→ 建议 **允许**  
2. **P1 组件**：表 + 图一起，还是先图？  
3. **画布**：12 列拖拽网格，还是先固定模板？  
