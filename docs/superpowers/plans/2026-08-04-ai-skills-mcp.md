...
- [x] Step 1: 写 `parse`/`List`/`ImportZip`/`SetEnabled`/`Get` 失败测试
- [x] Step 2: 实现最小存储 + 官方目录扫描 + zip 导入（防穿越）
- [x] Step 3: 挂路由并用 httptest 测 import/list/get/patch/delete
- [x] Step 4: `go test ./...` 通过并 commit
...
- [x] Step 1: 写配置 CRUD + 掩码 headers 测试
- [x] Step 2: 实现 JSON 文件存储；列表不回传明文 header value
- [x] Step 3: 实现最小 SSE/HTTP MCP：initialize + tools/list + tools/call（用 httptest mock server）
- [x] Step 4: 挂路由；`go test ./...`；commit
...
- [x] Step 1: client 增加 skills/mcp API 方法
- [x] Step 2: CapabilitiesPanel（双 Tab：导入/开关/预览；MCP CRUD/刷新）
- [x] Step 3: 侧栏「能力」按钮打开面板
- [x] Step 4: commit
...
- [x] Step 1: 注册 `list_skills`/`read_skill` 并实现 exec
- [x] Step 2: 发送前拉取 MCP tools 合并进 `tools`；MCP 调用走 `/api/ai/mcp/tools/call`
- [x] Step 3: system 提示注入已启用 Skill 目录摘要
- [x] Step 4: 单元测试 + commit
...
- [x] Step 1: 更新 DESIGN.md
- [x] Step 2: 手动/脚本冒烟：能力面板打开、skills list、mcp list
- [x] Step 3: push / 更新 PR
