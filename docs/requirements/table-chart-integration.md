...
> 图表能力依据：[`docs/features/charts/`](../features/charts/)（LabKey + Benchling 合并定稿）  
...
2. 在表的工具入口上提供六类图表创建与配置（对齐 `docs/features/charts`）；
...
以下均以 `docs/features/charts/*.md` 已确认项为准，只讨论**与表结合时的增量需求**。
...
| 图表功能域 | 文档 | 表侧前置 | 一体化增量 |
| --- | --- | --- | --- |
| 共性 CONFIGURE/STYLE | [common.md](../features/charts/common.md) | 字段元数据 | 工作区 Save、chartPosition |
| Bar | [bar-charts.md](../features/charts/bar-charts.md) | 分类/数值 | 视图行聚合、水平/堆叠 |
| Box | [box-plots.md](../features/charts/box-plots.md) | Y + Color/Shape | 分布统计；无 Jitter |
| Line + 拟合 | [line-charts.md](../features/charts/line-charts.md) | X/Y/Series | 拟合/打标/MODEL TABLES |
| Pie | [pie-charts.md](../features/charts/pie-charts.md) | Categories + Measure | 非 Count；无渐变 |
| Scatter + 拟合 | [scatter-plots.md](../features/charts/scatter-plots.md) | X/Y/Color/Shape/Size | 无密度；过密→Heatmap |
| Heatmap | [heatmaps.md](../features/charts/heatmaps.md) | 行列坐标+色值 | 色阶与 Hover |
...
4. 各图已确认的 CONFIGURE/STYLE/导出/拟合能力可按 `docs/features/charts` 逐项勾验。  
...
| 文档 | 作用 |
| --- | --- |
| [../features/charts/README.md](../features/charts/README.md) | **当前**图表功能点入口 |
| [../archive/labkey-charts/](../archive/labkey-charts/) | LabKey 溯源 |
| [../archive/benchling-charts/](../archive/benchling-charts/) | Benchling 溯源 |
...
> **以 vxe-table 为唯一业务数据入口，把 `docs/features/charts` 已确认的六类图表（含 Line/Scatter 拟合套件）挂到表工具链上，用字段元数据 + 当前视图行集驱动 CONFIGURE/STYLE/导出；图表相对表位置支持上/下/左/右。**
