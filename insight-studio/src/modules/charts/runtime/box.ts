...
import { AXIS_STYLE, baseLayout, displayVal, distinctInOrder, seriesColor, shapeFor } from './shared'
...
export function buildBoxOption({ result, config, viewName }: BuildInput): BuildOutput {
  ...
  colorVals.forEach((cv, ci) => {
    ...
    for (const cat of cats) {
      const subset = rows.filter((row) => (!xField || displayVal(row[xField]) === cat) && (cv === null || displayVal(row[colorField!]) === cv))
  ...
  return {
    option: {
      data,
      layout: {
        ...baseLayout(style, viewName ?? '', { legend: colorVals.length > 1 }),
        boxmode: 'group',
