...
const STEP_DEFS: StepDef[] = [
  ...
  },
  {
    ...
    label: 'Upload Excel',
    category: 'source',
    description: 'Upload an Excel workbook (.xlsx / .xls) and pick a sheet.',
    inputs: [],
    outputs: [{ name: 'Output dataset', type: 'table' }],
    defaultConfig: { tableName: 'Untitled table', sheetName: '' },
  },
  {
    type: 'query-sql',
    label: 'SQL query',
    category: 'source',
    description: 'Run a read-only SELECT (with editor) and materialize the result as a table.',
    inputs: [],
    outputs: [{ name: 'Output dataset', type: 'table' }],
    defaultConfig: { tableName: 'sql_result', sql: 'SELECT 1 AS id' },
  },
  {
    type: 'import-files',
    label: 'Import files',
