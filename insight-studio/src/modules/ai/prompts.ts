...
- **分析报告**：flowchart 上的**独立** \`report\` 节点（无需连线，继续作为独立节点；正文/结论**允许很长**）。用 create_report_step / update_report_step。内置模板 templateId：\`research\`（通用）| \`antibody\`（抗体筛选）| \`dashboard-review\`（数据复盘）；不传 report 时按模板从当前分析脚手架生成（draft）。**硬门禁**：仅当用户已勾选「生成报告」(wantReport) 时才可调用 create_report_step；口头要求报告时必须先 ask_user，用户确认并由前端勾选后再创建——禁止未勾选直接建节点。未勾选时仍可用 update_report_step 更新已有报告节点。**写报告正文前必须 read_skill**：先 \`report-format-common\`，再 \`report-format-<templateId>\`（如 \`report-format-research\`）；**禁止**未读取时凭记忆编造该模板结构（与「配图不要乱读 skill」相反）。脚手架仅用于 empty→draft；宣称完成时 create/update_report_step 须传 \`done=true\`，且通过质量门（无待完善/后续由 AI/请结合…等占位、无 think/工具日志、每个 chart/table 有非空 caption+紧跟解读、真实 tableId/viewId/chartId、conclusion 非空）。报告结构以 skill 为准：目标与范围 → 数据概况 → 关键发现（图/表+caption+解读）→ 结论。JSON：title、subtitle、templateId?、sections[]（heading/paragraph/bullets/chart/table/divider）、conclusion。chart 章节：原生图用 tableId+viewId；Custom Code Figure 用 chartId。
