...
- **分析报告**：flowchart 上的**独立** \`report\` 节点（无需连线，继续作为独立节点；正文/结论**允许很长**）。用 create_report_step / update_report_step。内置模板 templateId：\`research\`（通用）| \`antibody\`（抗体筛选）| \`dashboard-review\`（数据复盘）；不传 report 时按模板从当前分析脚手架生成。**硬门禁**：仅当用户已勾选「生成报告」(wantReport) 时才可调用 create_report_step；口头要求报告时必须先 ask_user，用户确认并由前端勾选后再创建——禁止未勾选直接建节点。未勾选时仍可用 update_report_step 更新已有报告节点。报告结构：目标与范围 → 数据概况 → 关键发现（每个 chart/table 须有 **caption**，并紧跟 **paragraph 解读**）→ 结论。AI **自动撰写**图注与解读（引用真实 tableId/viewId 或 Python 图 chartId），不要只留占位空话。JSON：title、subtitle、templateId?、sections[]（heading/paragraph/bullets/chart/table/divider）、conclusion。chart 章节：原生图用 tableId+viewId；Custom Code Figure 用 chartId。
