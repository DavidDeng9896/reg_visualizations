/**
 * Agent 工具实现（无 Pinia）：由 ToolWorkspace 提供分析文档与副作用。
 * 浏览器经 piniaWorkspace 绑定；dsh 经 HTTP workspace 绑定。
 */
...
import { validateChartMapping } from '../../charts/chartSlots'
...
function store() {
  ...
  return {
    ...
    mutate(fn: (analysis: Analysis) => void) {
      ws.mutate(fn)
    },
    load(id: string) {
      return ws.load(id)
...
function dashStore() {
  const ws = getWorkspace()
  return {
    loadList: () => ws.loadDashboardList(),
    loadOne: (id: string) => ws.loadDashboard(id),
    get currentId() {
      return ws.currentDashboardId
    },
  }
}
...
const impl: Record<string, (args: Record<string, unknown>, ctx: ToolCtx) => Promise<ToolExecResult> | ToolExecResult> = {
  async list_analyses() {
    const list = await getWorkspace().listAnalyses()
  ...
  async create_analysis(args) {
    ...
    await getWorkspace().putAnalysis(a)
  ...
  async list_ai_files() {
    try {
      const list = await getWorkspace().listFiles()
  ...
  async import_ai_file(args) {
    ...
    try {
      const imported = await getWorkspace().importAiFile(att, { tableNameHint: tableName, sheetNames })
  ...
  async refresh_sql_source(args) {
    ...
    try {
      const r = await getWorkspace().refreshSqlSource(stepId)
      return ok(
  ...
  async create_dashboard(args) {
    ...
    await getWorkspace().putDashboard(d)
  ...
  async add_dashboard_widget(args) {
    ...
    const d = await getWorkspace().getDashboard(dashboardId)
