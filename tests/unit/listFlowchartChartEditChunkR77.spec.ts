...
describe('List / Flowchart / ChartEdit cold-path re-eval (Round 77)', () => {
  it('keeps list route-lazy after CSV Cancel×toast + workspace empty CTA helpers', () => {
    expect(LIST_PAGE_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(listPageChunkStrategy().round77Reeval).toBe('keep-route-lazy')
  })

  it('keeps flowchart async-idle-warm after R77 a11y helpers', () => {
    expect(FLOWCHART_COLD_WARM_DEFERRED).toBe(true)
    expect(flowchartLoadMode()).toBe('async-idle-warm')
    expect(flowchartChunkStrategy().round77Reeval).toBe('keep-async-idle-warm')
  })

  it('keeps ChartEdit fitEngine/palette deferred-sync after Combine Cancel×toast + sidebar empty CTA', () => {
    ...
    expect(editDrawerChunkStrategy().round77Reeval).toBe('keep-deferred-sync')
  })
})
