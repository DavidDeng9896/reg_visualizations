...
import {
  ...
  sidebarEmptyCtaToastR60Regression,
} from '@/modules/sidebar/sidebarEmpty'
import { FOCUS_RESTORE_RING_CLASS } from '@/shared/ui/focusRestore'
import { toast } from '@/shared/ui/feedback'

describe('sidebar empty CTA × toast regression (Round 60)', () => {
  afterEach(() => {
    document.body.innerHTML = ''
    document.querySelectorAll('[data-ia-toast-host]').forEach((el) => el.remove())
  })

  it('documents Round 60 sidebar empty CTA × toast regression', () => {
    expect(sidebarEmptyCtaToastR60Regression()).toBe(true)
