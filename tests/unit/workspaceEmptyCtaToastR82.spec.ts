...
import {
  ...
  workspaceEmptyCtaToastR82Regression,
} from '@/modules/table/workspaceEmpty'
import { FOCUS_RESTORE_RING_CLASS } from '@/shared/ui/focusRestore'
import { toast } from '@/shared/ui/feedback'

describe('workspace empty CTA × toast regression (Round 82)', () => {
  afterEach(() => {
    document.body.innerHTML = ''
    document.querySelectorAll('[data-ia-toast-host]').forEach((el) => el.remove())
  })

  it('documents Round 82 workspace empty CTA × toast regression', () => {
    expect(workspaceEmptyCtaToastR82Regression()).toBe(true)
