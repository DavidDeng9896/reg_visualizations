...
describe('List / Create / CSV cold-path re-eval (Round 98)', () => {
  ...
  it('keeps Create async-idle-warm and Papa deferred-dynamic', () => {
    ...
    expect(createAnalysisChunkStrategy().round98Reeval).toBe('keep-async-idle-warm')
    expect(CSV_PAPAPARSE_STATIC_DEFERRED).toBe(true)
    expect(csvPapaLoadMode()).toBe('deferred-dynamic')
    expect(csvParseChunkStrategy().round98Reeval).toBe('keep-deferred-dynamic')
  })
})
