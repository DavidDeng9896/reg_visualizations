...
describe('List / Flowchart / ChartEdit cold-path re-eval (Round 74)', () => {
  it('keeps list route-lazy after CSV Cancel×toast + workspace empty CTA helpers', () => {
    ...
    expect(listPageChunkStrategy().round74Reeval).toBe('keep-route-lazy')
  })

  it('keeps flowchart async-idle-warm after R74 a11y helpers', () => {
    expect(FLOWCHART_COLD_WARM_DEFERRED).toBe(true)
    expect(flowchartLoadMode()).toBe('async-idle-warm')
    expect(flowchartChunkStrategy().round74Reeval).toBe('keep-async-idle-warm')
  })
  ...
  it('keeps ChartEdit fitEngine/palette deferred-sync after Cancel×toast spot-checks', () => {
    ...
    expect(editDrawerChunkStrategy().round74Reeval).toBe('keep-deferred-sync')
  })
})
