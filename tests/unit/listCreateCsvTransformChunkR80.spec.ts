...
describe('List / Create / CSV / Transform cold-path re-eval (Round 80)', () => {
  it('keeps list route-lazy after CSV/Combine Cancel×toast + empty CTA helpers', () => {
    expect(LIST_PAGE_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(listPageChunkStrategy().round80Reeval).toBe('keep-route-lazy')
  })
  ...
  it('keeps Create async-idle-warm, Papa deferred-dynamic, and Transform deferred-sync', () => {
    ...
    expect(createAnalysisChunkStrategy().round80Reeval).toBe('keep-async-idle-warm')
    expect(CSV_PAPAPARSE_STATIC_DEFERRED).toBe(true)
    expect(csvPapaLoadMode()).toBe('deferred-dynamic')
    expect(csvParseChunkStrategy().round80Reeval).toBe('keep-deferred-dynamic')
    expect(TRANSFORM_PIPELINE_SPLIT_DEFERRED).toBe(true)
    expect(transformPipelineSplitMode()).toBe('deferred-sync')
    expect(transformChunkStrategy().round80Reeval).toBe('keep-deferred-sync')
  })
})
