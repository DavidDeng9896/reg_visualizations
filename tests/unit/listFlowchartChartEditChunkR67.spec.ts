...
describe('List / Flowchart / ChartEdit cold-path re-eval (Round 67)', () => {
  it('keeps list route-lazy after Transform Esc×toast + workspace empty CTA helpers', () => {
    expect(LIST_PAGE_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(listPageChunkStrategy().round67Reeval).toBe('keep-route-lazy')
  })

  it('keeps flowchart async-idle-warm after R67 a11y helpers', () => {
    expect(FLOWCHART_COLD_WARM_DEFERRED).toBe(true)
    expect(flowchartLoadMode()).toBe('async-idle-warm')
    expect(flowchartChunkStrategy().round67Reeval).toBe('keep-async-idle-warm')
  })

  it('keeps ChartEdit fitEngine/palette deferred-sync after Cancel×toast regressions', () => {
    ...
    expect(editDrawerChunkStrategy().round67Reeval).toBe('keep-deferred-sync')
  })
})
