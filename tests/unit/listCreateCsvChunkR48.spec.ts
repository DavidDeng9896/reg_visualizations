...
describe('List / Create / CSV / projects cold-path re-eval (Round 48)', () => {
  it('keeps list route-lazy after landmark×filter + skip→Tab×filter helpers', () => {
    expect(LIST_PAGE_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(listPageChunkStrategy().round48Reeval).toBe('keep-route-lazy')
  })
  ...
  it('keeps Create async-idle-warm and Papa deferred-dynamic', () => {
    ...
    expect(createAnalysisChunkStrategy().round48Reeval).toBe('keep-async-idle-warm')
    expect(CSV_PAPAPARSE_STATIC_DEFERRED).toBe(true)
    expect(csvPapaLoadMode()).toBe('deferred-dynamic')
    expect(csvParseChunkStrategy().round48Reeval).toBe('keep-deferred-dynamic')
  })

  it('keeps projects shared entry after R48 a11y helpers', () => {
    expect(projectsChunkStrategy().round48Reeval).toBe('keep-shared')
  })
})
