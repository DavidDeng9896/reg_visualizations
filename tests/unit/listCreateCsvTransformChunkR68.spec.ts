...
describe('List / Create / CSV / Transform cold-path re-eval (Round 68)', () => {
  it('keeps list route-lazy after Create Cancel×toast + flowchart empty CTA helpers', () => {
    expect(LIST_PAGE_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(listPageChunkStrategy().round68Reeval).toBe('keep-route-lazy')
  })
  ...
  it('keeps Create async-idle-warm, Papa deferred-dynamic, and Transform deferred-sync', () => {
    ...
    expect(createAnalysisChunkStrategy().round68Reeval).toBe('keep-async-idle-warm')
    expect(CSV_PAPAPARSE_STATIC_DEFERRED).toBe(true)
    expect(csvPapaLoadMode()).toBe('deferred-dynamic')
    expect(csvParseChunkStrategy().round68Reeval).toBe('keep-deferred-dynamic')
    expect(TRANSFORM_PIPELINE_SPLIT_DEFERRED).toBe(true)
    expect(transformPipelineSplitMode()).toBe('deferred-sync')
    expect(transformChunkStrategy().round68Reeval).toBe('keep-deferred-sync')
  })
})
