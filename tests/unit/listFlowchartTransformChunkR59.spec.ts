...
describe('List / Flowchart / Transform cold-path re-eval (Round 59)', () => {
  it('keeps list route-lazy after Combine Esc×toast + flowchart CTA×toast helpers', () => {
    expect(LIST_PAGE_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(listPageChunkStrategy().round59Reeval).toBe('keep-route-lazy')
  })

  it('keeps flowchart async-idle-warm after R59 a11y helpers', () => {
    expect(FLOWCHART_COLD_WARM_DEFERRED).toBe(true)
    expect(flowchartLoadMode()).toBe('async-idle-warm')
    expect(flowchartChunkStrategy().round59Reeval).toBe('keep-async-idle-warm')
  })
  ...
  it('keeps Transform pipeline deferred-sync with idle warm on open', () => {
    ...
    expect(transformChunkStrategy().round59Reeval).toBe('keep-deferred-sync')
  })
})
