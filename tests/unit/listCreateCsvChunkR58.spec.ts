...
describe('List / Create / CSV cold-path re-eval (Round 58)', () => {
  it('keeps list route-lazy after Create Esc×toast + workspace empty CTA helpers', () => {
    ...
    expect(listPageChunkStrategy().round58Reeval).toBe('keep-route-lazy')
  })
  ...
  it('keeps Create async-idle-warm and Papa deferred-dynamic', () => {
    ...
    expect(createAnalysisChunkStrategy().round58Reeval).toBe('keep-async-idle-warm')
    expect(CSV_PAPAPARSE_STATIC_DEFERRED).toBe(true)
    expect(csvPapaLoadMode()).toBe('deferred-dynamic')
    expect(csvParseChunkStrategy().round58Reeval).toBe('keep-deferred-dynamic')
  })
})
