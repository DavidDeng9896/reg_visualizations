...
describe('List / Create / CSV cold-path re-eval (Round 52)', () => {
  it('keeps list route-lazy after workspace empty CTA×toast + skip→empty Tab + Demo focus', () => {
    expect(LIST_PAGE_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(listPageChunkStrategy().round52Reeval).toBe('keep-route-lazy')
  })
  ...
  it('keeps Create async-idle-warm and Papa deferred-dynamic', () => {
    ...
    expect(createAnalysisChunkStrategy().round52Reeval).toBe('keep-async-idle-warm')
    expect(CSV_PAPAPARSE_STATIC_DEFERRED).toBe(true)
    expect(csvPapaLoadMode()).toBe('deferred-dynamic')
    expect(csvParseChunkStrategy().round52Reeval).toBe('keep-deferred-dynamic')
  })
})
