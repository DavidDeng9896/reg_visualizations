...
import {
  ...
  sidebarEmptyCtaToastR57SpotCheck,
} from '@/modules/sidebar/sidebarEmpty'
import { FOCUS_RESTORE_RING_CLASS } from '@/shared/ui/focusRestore'
import { toast } from '@/shared/ui/feedback'

describe('sidebar empty CTA × toast spot-check (Round 57)', () => {
  afterEach(() => {
    document.body.innerHTML = ''
    document.querySelectorAll('[data-ia-toast-host]').forEach((el) => el.remove())
  })

  it('documents Round 57 sidebar empty CTA × toast spot-check regression', () => {
    expect(sidebarEmptyCtaToastR57SpotCheck()).toBe(true)
