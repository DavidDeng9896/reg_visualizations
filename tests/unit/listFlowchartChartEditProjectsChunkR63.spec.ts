import { describe, expect, it } from 'vitest'
import {
  LIST_PAGE_CHUNK_SPLIT_DEFERRED,
  listPageChunkStrategy,
} from '@/modules/analysis/listPageChunk'
import {
  FLOWCHART_COLD_WARM_DEFERRED,
  flowchartChunkStrategy,
  flowchartLoadMode,
} from '@/modules/flowchart/flowchartChunk'
import {
  EDIT_DRAWER_FIT_ENGINE_SPLIT_DEFERRED,
  EDIT_DRAWER_PALETTE_SPLIT_DEFERRED,
  editDrawerChunkStrategy,
  editDrawerFitEngineSplitMode,
  editDrawerPaletteSplitMode,
} from '@/modules/chart/editDrawerChunk'
import {
  PROJECTS_CHUNK_SPLIT_DEFERRED,
  projectsChunkStrategy,
} from '@/shared/mock/projectsChunk'

describe('List / Flowchart / ChartEdit / projects cold-path re-eval (Round 63)', () => {
  it('keeps list route-lazy after CSV Esc×toast + workspace skip→empty Tab helpers', () => {
    expect(LIST_PAGE_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(listPageChunkStrategy().round63Reeval).toBe('keep-route-lazy')
  })

  it('keeps flowchart async-idle-warm after R63 a11y helpers', () => {
    expect(FLOWCHART_COLD_WARM_DEFERRED).toBe(true)
    expect(flowchartLoadMode()).toBe('async-idle-warm')
    expect(flowchartChunkStrategy().round63Reeval).toBe('keep-async-idle-warm')
  })

  it('keeps ChartEdit fitEngine/palette deferred-sync after Esc×toast spot-checks', () => {
    expect(EDIT_DRAWER_FIT_ENGINE_SPLIT_DEFERRED).toBe(true)
    expect(EDIT_DRAWER_PALETTE_SPLIT_DEFERRED).toBe(true)
    expect(editDrawerFitEngineSplitMode()).toBe('deferred-sync')
    expect(editDrawerPaletteSplitMode()).toBe('deferred-sync')
    expect(editDrawerChunkStrategy().round63Reeval).toBe('keep-deferred-sync')
  })

  it('keeps projects shared after sidebar empty CTA×toast regression', () => {
    expect(PROJECTS_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(projectsChunkStrategy().round63Reeval).toBe('keep-shared')
  })
})
