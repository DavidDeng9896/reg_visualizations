...
describe('List / Flowchart / ChartEdit / projects cold-path re-eval (Round 69)', () => {
  it('keeps list route-lazy after ChartEdit Esc×toast + workspace skip→empty Tab helpers', () => {
    expect(LIST_PAGE_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(listPageChunkStrategy().round69Reeval).toBe('keep-route-lazy')
  })

  it('keeps flowchart async-idle-warm after R69 a11y helpers', () => {
    expect(FLOWCHART_COLD_WARM_DEFERRED).toBe(true)
    expect(flowchartLoadMode()).toBe('async-idle-warm')
    expect(flowchartChunkStrategy().round69Reeval).toBe('keep-async-idle-warm')
  })

  it('keeps ChartEdit fitEngine/palette deferred-sync after Esc×toast spot-checks', () => {
    ...
    expect(editDrawerChunkStrategy().round69Reeval).toBe('keep-deferred-sync')
  })

  it('keeps projects shared after Transform Cancel×toast + list empty CTA×toast', () => {
    expect(PROJECTS_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(projectsChunkStrategy().round69Reeval).toBe('keep-shared')
  })
})
