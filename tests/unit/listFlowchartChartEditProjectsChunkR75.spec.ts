...
describe('List / Flowchart / ChartEdit / projects cold-path re-eval (Round 75)', () => {
  it('keeps list route-lazy after CSV Esc×toast + workspace skip→empty Tab helpers', () => {
    ...
    expect(listPageChunkStrategy().round75Reeval).toBe('keep-route-lazy')
  })

  it('keeps flowchart async-idle-warm after R75 a11y helpers', () => {
    expect(FLOWCHART_COLD_WARM_DEFERRED).toBe(true)
    expect(flowchartLoadMode()).toBe('async-idle-warm')
    expect(flowchartChunkStrategy().round75Reeval).toBe('keep-async-idle-warm')
  })
  ...
  it('keeps ChartEdit fitEngine/palette deferred-sync after Esc×toast spot-checks', () => {
    ...
    expect(editDrawerChunkStrategy().round75Reeval).toBe('keep-deferred-sync')
  })

  it('keeps projects shared after Combine Esc×toast + list empty CTA×toast', () => {
    expect(PROJECTS_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(projectsChunkStrategy().round75Reeval).toBe('keep-shared')
  })
})
