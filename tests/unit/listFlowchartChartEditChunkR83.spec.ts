...
describe('List / Flowchart / ChartEdit cold-path re-eval (Round 83)', () => {
  it('keeps list route-lazy after Transform Cancel×toast + flowchart empty CTA helpers', () => {
    ...
    expect(listPageChunkStrategy().round83Reeval).toBe('keep-route-lazy')
  })

  it('keeps flowchart async-idle-warm after R83 a11y helpers', () => {
    expect(FLOWCHART_COLD_WARM_DEFERRED).toBe(true)
    expect(flowchartLoadMode()).toBe('async-idle-warm')
    expect(flowchartChunkStrategy().round83Reeval).toBe('keep-async-idle-warm')
  })
  ...
  it('keeps ChartEdit fitEngine/palette deferred-sync after Esc×toast + New view Cancel', () => {
    ...
    expect(editDrawerChunkStrategy().round83Reeval).toBe('keep-deferred-sync')
  })
})
