...
describe('List / Create / CSV cold-path re-eval (Round 108)', () => {
  it('keeps list route-lazy after CSV/Combine Esc×toast + empty CTA + L-05 hint dedupe', () => {
    expect(LIST_PAGE_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(listPageChunkStrategy().round108Reeval).toBe('keep-route-lazy')
  })
  ...
  it('keeps Create async-idle-warm and Papa deferred-dynamic', () => {
    ...
    expect(createAnalysisChunkStrategy().round108Reeval).toBe('keep-async-idle-warm')
    expect(CSV_PAPAPARSE_STATIC_DEFERRED).toBe(true)
    expect(csvPapaLoadMode()).toBe('deferred-dynamic')
    expect(csvParseChunkStrategy().round108Reeval).toBe('keep-deferred-dynamic')
  })
})
