...
describe('List / Create / CSV cold-path re-eval (Round 50)', () => {
  it('keeps list route-lazy after empty CTA×toast + filter-focus preserve helpers', () => {
    expect(LIST_PAGE_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(listPageChunkStrategy().round50Reeval).toBe('keep-route-lazy')
  })
  ...
  it('keeps Create async-idle-warm and Papa deferred-dynamic', () => {
    ...
    expect(createAnalysisChunkStrategy().round50Reeval).toBe('keep-async-idle-warm')
    expect(CSV_PAPAPARSE_STATIC_DEFERRED).toBe(true)
    expect(csvPapaLoadMode()).toBe('deferred-dynamic')
    expect(csvParseChunkStrategy().round50Reeval).toBe('keep-deferred-dynamic')
  })
})
