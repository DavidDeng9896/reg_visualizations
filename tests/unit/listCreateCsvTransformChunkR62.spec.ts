...
describe('List / Create / CSV / Transform cold-path re-eval (Round 62)', () => {
  it('keeps list route-lazy after Create Cancel×toast + empty CTA helpers', () => {
    expect(LIST_PAGE_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(listPageChunkStrategy().round62Reeval).toBe('keep-route-lazy')
  })
  ...
  it('keeps Create async-idle-warm, Papa deferred-dynamic, and Transform deferred-sync', () => {
    ...
    expect(createAnalysisChunkStrategy().round62Reeval).toBe('keep-async-idle-warm')
    expect(CSV_PAPAPARSE_STATIC_DEFERRED).toBe(true)
    expect(csvPapaLoadMode()).toBe('deferred-dynamic')
    expect(csvParseChunkStrategy().round62Reeval).toBe('keep-deferred-dynamic')
    expect(TRANSFORM_PIPELINE_SPLIT_DEFERRED).toBe(true)
    expect(transformPipelineSplitMode()).toBe('deferred-sync')
    expect(transformChunkStrategy().round62Reeval).toBe('keep-deferred-sync')
  })
})
