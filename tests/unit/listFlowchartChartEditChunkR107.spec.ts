...
describe('List / Flowchart / ChartEdit cold-path re-eval (Round 107)', () => {
  it('keeps list route-lazy after Transform Cancel×toast + flowchart empty CTA + L-05 live', () => {
    expect(LIST_PAGE_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(listPageChunkStrategy().round107Reeval).toBe('keep-route-lazy')
  })

  it('keeps flowchart async-idle-warm after R107 a11y helpers', () => {
    expect(FLOWCHART_COLD_WARM_DEFERRED).toBe(true)
    expect(flowchartLoadMode()).toBe('async-idle-warm')
    expect(flowchartChunkStrategy().round107Reeval).toBe('keep-async-idle-warm')
  })
  ...
  it('keeps ChartEdit fitEngine/palette deferred-sync after Esc×toast + New view Cancel', () => {
    ...
    expect(editDrawerChunkStrategy().round107Reeval).toBe('keep-deferred-sync')
  })
})
