...
describe('List / Flowchart / ChartEdit / projects cold-path re-eval (Round 81)', () => {
  it('keeps list route-lazy after Transform Esc×toast + flowchart empty CTA helpers', () => {
    expect(LIST_PAGE_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(listPageChunkStrategy().round81Reeval).toBe('keep-route-lazy')
  })

  it('keeps flowchart async-idle-warm after R81 a11y helpers', () => {
    expect(FLOWCHART_COLD_WARM_DEFERRED).toBe(true)
    expect(flowchartLoadMode()).toBe('async-idle-warm')
    expect(flowchartChunkStrategy().round81Reeval).toBe('keep-async-idle-warm')
  })

  it('keeps ChartEdit fitEngine/palette deferred-sync after Cancel×toast spot-checks', () => {
    ...
    expect(editDrawerChunkStrategy().round81Reeval).toBe('keep-deferred-sync')
  })

  it('keeps projects shared after ChartEdit Cancel×toast + list empty CTA×toast', () => {
    expect(PROJECTS_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(projectsChunkStrategy().round81Reeval).toBe('keep-shared')
  })
})
