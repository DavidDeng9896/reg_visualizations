...
describe('List / Flowchart / Transform cold-path re-eval (Round 47)', () => {
  it('keeps list route-lazy after landmark migrate + skip→Tab helpers', () => {
    expect(LIST_PAGE_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(listPageChunkStrategy().round47Reeval).toBe('keep-route-lazy')
  })

  it('keeps flowchart async-idle-warm after workspace mount (cold path)', () => {
    expect(FLOWCHART_COLD_WARM_DEFERRED).toBe(true)
    expect(flowchartLoadMode()).toBe('async-idle-warm')
    expect(flowchartChunkStrategy().round47Reeval).toBe('keep-async-idle-warm')
  })
  ...
  it('keeps Transform pipeline deferred-sync with idle warm on open', () => {
    ...
    expect(transformChunkStrategy().round47Reeval).toBe('keep-deferred-sync')
  })
})
