...
import {
  ...
  workspaceEmptyCtaToastR58SpotCheck,
} from '@/modules/table/workspaceEmpty'
import { FOCUS_RESTORE_RING_CLASS } from '@/shared/ui/focusRestore'
import { toast } from '@/shared/ui/feedback'

describe('workspace empty CTA × toast spot-check (Round 58)', () => {
  afterEach(() => {
    document.body.innerHTML = ''
    document.querySelectorAll('[data-ia-toast-host]').forEach((el) => el.remove())
  })

  it('documents Round 58 workspace empty CTA × toast spot-check regression', () => {
    expect(workspaceEmptyCtaToastR58SpotCheck()).toBe(true)
