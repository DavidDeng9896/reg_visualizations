...
describe('List / Flowchart / ChartEdit cold-path re-eval (Round 57)', () => {
  it('keeps list route-lazy after New view Esc×toast + skip→empty landmark helpers', () => {
    expect(LIST_PAGE_CHUNK_SPLIT_DEFERRED).toBe(true)
    expect(listPageChunkStrategy().round57Reeval).toBe('keep-route-lazy')
  })

  it('keeps flowchart async-idle-warm after R57 a11y helpers', () => {
    expect(FLOWCHART_COLD_WARM_DEFERRED).toBe(true)
    expect(flowchartLoadMode()).toBe('async-idle-warm')
    expect(flowchartChunkStrategy().round57Reeval).toBe('keep-async-idle-warm')
  })

  it('keeps ChartEdit fitEngine/palette deferred-sync after Esc×toast', () => {
    ...
    expect(editDrawerChunkStrategy().round57Reeval).toBe('keep-deferred-sync')
  })
})
