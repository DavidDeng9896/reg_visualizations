...
├─────────────────────────────────────────────────────────┤
│  Toolbar                                                │
│  [列设置] [筛选/排序状态] [导出表] │ [创建图表 ▾] [已创建图表]   │
├───────────────────────────────┬─────────────────────────┤
│  主视图：vxe-table 数据表        │  可选：图表预览区 / 抽屉     │
│  - 行数据浏览                   │  - Chart Type 配置        │
│  - 列类型感知                   │  - Chart Layout 配置      │
│  - 筛选 / 排序 / 分页            │  - 图表面板切换            │
└───────────────────────────────┴─────────────────────────┘
         │ 当前视图数据（schema + rows）
         ▼
...
| C-06 | 视图切换 | 建议支持「表 / 图」切换或上下分栏；至少支持图预览不丢表状态 |
...
| ChartEntry | 「创建图表」菜单与已打开图列表 | — |
| ChartTypeDialog | 五类图的数据映射配置（拖拽） | features 细化项 |
| ChartLayoutPanel | 外观/轴/误差棒等 | features 细化项 |
...
5. **表/图布局**：上下分栏、左右分栏，还是整页切换？
6. **是否需要多图表实例并存**（同一表多个已创建图）？
...
> **以 vxe-table 数据表为唯一业务数据入口，把已确认的五类图表能力挂到表工具链上，用统一的字段元数据 + 当前视图行集驱动 Chart Type/Layout/导出。**

确认 §9 待决问题后，可进入技术方案（表状态管理、图表配置模型、渲染引擎选型）与实现计划。
